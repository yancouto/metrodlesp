...
export interface Station { id: string; name: string; lines: LineId[]; }
...
export function initKeyboard(opts: {
  ...
  hyphen.type = 'button'; hyphen.className = 'key dash'; hyphen.textContent = '-'; hyphen.dataset.value = '-';
  hyphen.addEventListener('click', () => handleKey('-'));
  utilRow.appendChild(hyphen);

  const space = document.createElement('button');
  space.type = 'button'; space.className = 'key action wide'; space.textContent = 'Espaço'; space.dataset.value = ' ';
  ...
  const api = {
    update() {
      ...
      for (const b of buttons) {
        ...
        if (v === 'BACKSPACE' || v === 'OK') { b.dataset.disabled = 'false'; continue; }
